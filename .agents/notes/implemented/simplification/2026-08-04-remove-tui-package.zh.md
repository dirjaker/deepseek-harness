...
删除旧 TUI workspace，不提供兼容包或别名。其源码、包测试、终端快照、依赖声明、已打补丁的 `pi-tui` 产物、workspace 引用、生成的服务目录条目和文档会一并移除。通用宿主能力与 agent loop（智能体循环）能力保持不变。
...
本记录汇总了因移除该包而无法继续保持现行有效、现已删除的仅涉及该包的记录。终端 UI 曾在长对话期间保持会话身份可见、移除重复模型标签、为消息附加耗时与阶段状态、在提示词旁显示 workspace 与分支上下文，并保守地解析完整 XML 包装层，以生成人类可读的回退输出。这些选择改善了一个终端前端，但在没有部署的情况下，并不足以证明应保留它。后来的[行模式 TUI profile](../feature/2026-08-17-line-oriented-tui-profile.md) 将同一个包名用于更小的 bundle，并不会恢复已删除的全终端渲染器。未来的 XML 回退仍必须使用真正的解析器而非正则表达式。
...
已删除的全终端 UI 包仍不存在。现有 import 和依赖该实现的 `cordis.yml` 条目会直接失败，不会得到兼容转换。Web 仍是已交付的浏览器表层；ACP、JSON-RPC、一次性 CLI 与独立的行模式 TUI 是 Web 之外的入口。
