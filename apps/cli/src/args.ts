 ...
 *
 * `web` and `tui` are hardcoded aliases for their matching profiles; `plugin`
...
Examples:
  ...
  dsh tui                                    start an interactive terminal session
...
export function parseDshArgs(argv: readonly string[], version: string): DshInvocation {
  ...
  const tui = program.command('tui').description('boot the tui profile (alias of --profile tui); the terminal app\'s own flags follow')
  tui
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
