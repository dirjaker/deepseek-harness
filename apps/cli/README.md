...
| `dsh tui` | Alias of `--profile tui`; start an interactive line-oriented terminal session. |
...
dsh tui "summarize this repository" # optional first task; /exit quits the REPL
...
Bundles named in `dsh.profile.bundles` resolve from the dsh installation first (`@deepseek-ai/dsh-base`, `@deepseek-ai/dsh-web-app`, `@deepseek-ai/dsh-headless`, `@deepseek-ai/dsh-tui`), then from the profile's own `node_modules`, where pnpm installs out-of-tree plugins.
