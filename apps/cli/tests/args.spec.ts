...
describe('parseDshArgs', () => {
  ...
  it('ends the launcher flags at the first token it does not own', () => {
    ...
    expect(parse(['tui', '--prompt', 'ask> ', 'hello']))
      .toEqual({ mode: 'profile', profile: 'tui', patches: [], args: ['--prompt', 'ask> ', 'hello'] })
  ...
  it('rejects missing profile, removed flags, and contradictory inputs', () => {
    ...
    expect(exitCode(['tui', '--dump-default-config', '--patch', 'w.yml'])).toBe(1)
