...
| `runProfile` / `composeProfileForRun` / `prepareProfileForRun` / `createProcessShutdown` | CLI 与嵌入式 app host 共享的 profile runner；调用方提供安装锚点、环境快照、app 参数、可选随包 preset root，以及可选的挂载前服务准备 |
